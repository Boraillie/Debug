...
static void _SetDefaultMaster(void)
{
    ...
    pMatrix->MATRIX_SCFG[2] |= MATRIX_SCFG_FIXED_DEFMSTR(0) | /* Master 0 */
                               MATRIX_SCFG_DEFMSTR_TYPE(2);   /* Fixed Default Master */
}
void portable_init(core_portable *p, int *argc, char *argv[])
{
    ...
    //printf("-- Basic Coremark Project %s --\n\r", SOFTPACK_VERSION);
    //printf("-- %s\n\r", BOARD_NAME);
    //printf("-- Compiled: %s %s --\n\r", __DATE__, __TIME__);
    ...
    int i;
    for (i=0; i<*argc; i++) {
        sprintf(sBuffer,"Arg[%d]=%s\n\r",i,argv[i]);
        UART_Printk(sBuffer,UART0);
    }
#endif
    if (sizeof(ee_ptr_int) != sizeof(ee_u8 *)) {
        UART_Printk("ERROR! Please define ee_ptr_int to a type that holds a pointer!\n\r",UART0);
    }
    if (sizeof(ee_u32) != 4) {
        UART_Printk("ERROR! Please define ee_u32 to a 32b unsigned type!\n\r",UART0);
    }
#if (MAIN_HAS_NOARGC && (SEED_METHOD==SEED_ARG))
    UART_Printk("ERROR! Main has no argc, but SEED_METHOD defined to SEED_ARG!\n\r",UART0);
#endif

    _SetDefaultMaster();
    currentMckFreq = gMCK;//CoreBoardConfig();
    ...
    /* Configures Timer Counter 0 (TC0) to generate an interrupt every 1 ms. */
    //PMC_EnablePeripheral(ID_TC0);
    PMC->PMC_PCER0 |= ((1 << ID_TC0));
    TC_Configure(TC0, 0, TC_CMR_TCCLKS_TIMER_CLOCK2 | TC_CMR_CPCTRG);
    temp = currentMckFreq / 8 / 1000;
    if ( (temp * 8 * 1000 ) != currentMckFreq)
    {
        UART_Printk("Warning: The accuracy of TC is not good.\n\r", UART0);
    }
    REG_TC0_RC0 = temp;
    REG_TC0_IER0 = TC_IER_CPCS;

    UART_Printk("-- CoreMark is running. Please wait...\n\r", UART0);
...
{
    UART_Printk("-- End of CoreMark test.\n\r",UART0);
    p->portable_id=0;
}
