...
// ---- Test implementation --------------------------------------------------
//#warning This is a dummy test created for development purpose only.
...
_PRG_TEST_FUNC_DECLARE(prg_non_reg_test)
{
 uint32_t  i,j;
...
//#warning debug code
  ...
  if(test_parameter1==0){ /////////////////////////////// non reg mode 0//
    ...
    for(j=0;j<test_parameter1;++j){
      for(i=0;i<test_parameter2;++i){
        ci=(unsigned char)i;
        large_results_array[i]=(ci<<24)|(((ci+1)%256)<<16)|(((ci+2)%256)<<8)|((ci+3)%256);
        }
      }
  ...
  for(i=2;i<TEST_MAX_RESULTS32;++i){ //fill all results except reserved 0 & 1
    ci=(unsigned char)i;
    CURRENT_TEST_RESULT(i)=(ci<<24)|(ci<<16)|(ci<<8)|(ci);
    }
